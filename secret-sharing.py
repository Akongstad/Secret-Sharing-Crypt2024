# File is written for Python 3
# If you want to visualize the shares as a polynomial make sure to 1. Install matplotlib and 2. Uncomment the import
#       statement, 3. Uncomment the plot_shares function. 4.Uncomment the plot_shares function call in main.

# To run the code, use cli and type: "python3 .\secret-sharing.py"
# The code is tested and are assumed to be working as expected.
# See comments in the code for more information.
# The file opens with constants and field operations, then the split_secret function, reconstruct function and test function.

# (Note): Function take default param F=F to allow for testing with different fields.

import secrets

# import matplotlib.pyplot as plt

# CONSTS
F_HEX = ""
F = int(F_HEX, 16)

SHARE_1 = int(
    "",
    16)
SHARE_2 = int(
    "",
    16)
SHARE_4 = int(
    "",
    16)

# N = 5, k = 3
SHARES = [(1, SHARE_1), (2, SHARE_2), (4, SHARE_4)]


# Field operations (to be used in conjunction with pow() for exponentiation)
def F_add(a, b, F=F) -> int:
    """
    Add two numbers in the field.
    :param a: First number.
    :param b: Second number.
    :return: a + b.
    """
    return (a + b) % F


def F_sub(a, b, F=F) -> int:
    """
    Subtract two numbers in the field.
    :param a: First number.
    :param b: Second number.
    :return: a - b.
    """
    return (a - b) % F


def F_mul(a, b, F=F) -> int:
    """
    Multiply two numbers in the field.
    :param a: First number.
    :param b: Second number.
    :return: a * b.
    """
    return (a * b) % F


def F_div(a, b, F=F) -> int:
    """
    Divide two numbers in the field.
    :param a: First number.
    :param b: Second number.
    :return: a / b.
    """
    return F_mul(a, pow(b, -1, F))


def sample_coefficients(k, F=F) -> []:
    """sample k coefficients from F. (Use secrets for cryptographically secure random numbers.)"""
    for i in range(k):
        yield secrets.randbelow(F)  # Q: Can these be < 0?


def split_secret(S: str, N, k, F=F) -> []:
    """
    Split the secret S into N shares
    :param S: The secret to split. (bit string)
    :param N: # of shares to split into.
    :param k: # shares for reconstruction. I.e. deg(f) = k - 1.
    :param F: The field to operate in.
    :return: List of N shares with k needed for reconstruction. Def Share:= (i, f(i)).
    """
    s = int(S, 2)
    as_ = [s] + list(sample_coefficients(k - 1, F))  # Random coefficients.

    def f_(x):  # Polynomial definition of the form:  f(x) = S + a_1x + a_2x^2 + ... + a_k-1 * x^k-1
        res = 0
        for i, a in enumerate(as_):
            res = F_add(res, F_mul(a, pow(x, i, F), F), F)
        return res

    shares = [(i, f_(i)) for i in range(1, N + 1)]  # (1, f(1)), ..., (N, f(N))
    return shares


def reconstruct(shares: [], degree, x, F=F) -> int:
    """
    Evaluate the polynomial of degree=degree interpolated from shares at x.
    :param shares: List of shares. **Shares**: (i, f(i)). **Invariance**: 1. Len(shares) >= k, 2. i is unique, 3. f(i) \in F.
    :param degree: The degree of the polynomial.
    :param x: The x value to evaluate the polynomial at.
    :param F: The field to operate in.
    :return: f(x) - value of polynomial at x.
    """
    k = degree + 1

    if len(shares) < k:
        raise ValueError("Not enough shares to reconstruct the polynomial.")

    secret = 0

    # Lagrange interpolation
    for i in range(k): # f(x) = \sum(f_i * y_i), where y_i = \prod((x - x_j) / (x_i - x_j)), j != i
        x_i, f_i = shares[i]
        y_i = 1
        for j in range(k): # Compute y_i = (x - x_j) / (x_i - x_j)
            if i == j:
                continue
            x_j, _ = shares[j]
            y_i = F_mul(F_div(F_sub(x, x_j, F), F_sub(x_i, x_j, F), F), y_i, F)

        y_i = F_mul(y_i, f_i, F) # f_i * y_i
        secret = F_add(secret, y_i, F) # secret += y_i

    return secret


def test():
    print("Running tests...")
    # Assertions
    # Field operations
    assert F_add(1, 2) == 3
    assert F_sub(1, 2) == F - 1
    assert F_mul(2, 3) == 6
    assert F_div(6, 3) == 2

    # Split secret
    S = 42
    S_bin = bin(S)[2:]  # [2:] to remove the '0b' prefix. 101010
    shares = split_secret(S_bin, 5, 3)
    assert len(shares) == 5  # split_secret return N shares.
    assert all([1 <= i <= 5 for i, _ in shares])  # Shares are deterministic and in range
    assert all([a <= F for a in sample_coefficients(5)])  # Random coefficients are in range.

    # Reconstruct secret
    assert reconstruct(shares[:3], 2, 0) == 42  # Reconstruct the secret from the shares.
    assert reconstruct(SHARES, 2, 1) == SHARE_1  # Reconstruct the secret from the shares.

    # Check we can reconstruct SHARE_4 using constructed SHARE_0 with SHARE_1 and SHARE_2
    assert reconstruct([(0, reconstruct(SHARES, 2, 0)), (1, SHARE_1), (2, SHARE_2)], 2, 4) == SHARE_4

    print("All tests passed.\n")


# def plot_shares(shares, S):
#     plt.scatter([0] + [i for i, _ in shares], [S] + [f for _, f in shares])
#     plt.xlabel("x")
#     plt.ylabel("f(x)")
#     plt.show()


if __name__ == '__main__':
    test()
    # Example 1
    print("Running Sample Run 1. N=5, k=3, F=101, S=42...")
    S = 42
    S_bin = bin(S)[2:]  # [2:] to remove the '0b' prefix. 101010
    print(f"The secret is: {S} == 101010")

    shares = split_secret(S_bin, 5, 3, 101)
    print("The secret is split into 5 shares with 3 needed for reconstruction.")
    print("Shares: ", shares)

    print(f"Reconstructing the secret using the first 3 shares: {shares[:3]}")
    reconstructed_secret = reconstruct(shares[:3], 2, 0, 101)
    assert reconstructed_secret == S
    print(f"The reconstructed secret is: {reconstructed_secret} == {S}\n")
    # Plot the shares as points.
    # plot_shares(shares, S)

    # Examples 2
    print(f"Running Sample Run 2. N=5, k=3, F={F_HEX}, S=?...")
    share_0 = reconstruct(SHARES, 2, 0)
    print("The reconstructed secret/share_0 is: ", hex(share_0)[2:])
